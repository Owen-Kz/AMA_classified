...
const category = GetParameters(window.location.href).get("c")
...
function booksNavigation(totalPagesListings, currentPage) {
    ...
    if (totalPagesListings > 0) {
      if (currentPage > 1) {
        Previous = `
         <a href="?c=${category}&page=${currentPage - 1}" class="pagination_item"><li>Prev</li></a>
        `;
      }
      ...
      if (startPage > 1) {
        AfterPrevious = `
        <a href="?c=${category}&page=1" class="pagination_item"><li>1</li></a>
        `;
      ...
      for (let i = startPage; i <= totalPagesListings; i++) {
        ...
        OtherPages += `
         <a href="?c=${category}&page=${i}" class="pagination_item ${active}"><li>${i}</li></a>
        `;
      }
      ...
      if (endPage <= totalPagesListings) {
        ...
        TotalPagesCount = `
        <a href="?c=${category}&page=${totalPagesListings}" class="pagination_item "><li>${totalPagesListings}</li></a>
        `;
      }
  
      if (currentPage < totalPagesListings) {
        nextPageContainer = `
        <a href="?c=${category}&page=${nextPage}" class="pagination_item "><li>Next</li></a>
        `;
      }
    }
...
fetch(`/itemsInCat?page=${page}&cat=${category}`, {
    method:"POST"
}).then(res=>res.json())
.then(async (data) =>{
    ...
    if(data.success){
        ...
        console.log(totalPages, currentPage, ListingsList.length)
