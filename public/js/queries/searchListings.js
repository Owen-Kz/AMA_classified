...
const query = GetParameters(window.location.href).get("q")
...
function booksNavigation(totalPagesListings, currentPage) {
    ...
    if (totalPagesListings > 0) {
      ...
      if (startPage > 1) {
        AfterPrevious = `
        <a href="?page=1&q=${query}" class="pagination_item"><li>1</li></a>
        `;
      ...
      for (let i = startPage; i <= totalPagesListings; i++) {
        ...
        OtherPages += `
         <a href="?page=${i}&q=${query}" class="pagination_item ${active}"><li>${i}</li></a>
        `;
      }
      ...
      if (endPage <= totalPagesListings) {
        ...
        TotalPagesCount = `
        <a href="?page=${totalPagesListings}&q=${query}" class="pagination_item "><li>${totalPagesListings}</li></a>
        `;
      }
  
      if (currentPage < totalPagesListings) {
        nextPageContainer = `
        <a href="?page=${nextPage}&q=${query}" class="pagination_item "><li>Next</li></a>
        `;
      }
    }
...
fetch(`/listings?page=${page}&q=${query}`, {
    method:"POST"
}).then(res=>res.json())
.then(async (data) =>{
