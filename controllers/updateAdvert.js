...
const UpdateAdvert = (req, res) => {
  ...
  upload.fields([
    ...
    const {itemID, title, description, price, category, subCategories, videoURL, country, condition, purpose, oldThumbnail, oldVIdeoFile} = req.body;
...
const data = {
        ...
        //   imageFiles: uploadedImageUrls,
          videoFile: videoFile ? uploadedVideoUrl : oldVIdeoFile,
          thumbnail: thumbnail ? uploadedThumbnailUrl : oldThumbnail,
        }; 

        const response = await fetch(`${process.env.ENDPOINT}/y/updateAdvert`, {
          method: 'POST',
          headers: {
